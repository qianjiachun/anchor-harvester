...
export default defineComponent({
    ...
    setup(props, ctx) {
        ...
        let radioType = ref(0) as Ref<number>;
        let isTableLoading = ref(false) as Ref<Boolean>;
        ...
        const onChangeRadio = (label: number) => {
            postData.type = label;
            getTaskDetailList(postData);
        }

        const getTaskDetailList = (data: ITaskData) => {
            isTableLoading.value = true;
            getTaskDetail(data).then(ret => {
                if (ret.data.error == 0) {
        ...
        const exportExcel = (sociatyType: number) => {
            ...
            let name = sociatyType == 0 ? "未入会" : "非铂金公会";
            exportJsonToExcel(`【虎牙】【${name}】【${zone1}】未入会主播名单`, header, body);
        }
        ...
        return () => (
            <>
                <el-button style="margin-bottom:15px;" type="primary" onClick={() => {exportExcel(radioType.value)}}>导出excel</el-button>
                <br/>
                <el-radio-group style="margin-bottom:15px;" v-model={radioType.value} onChange={onChangeRadio}>
                    <el-radio label={0}>未入会</el-radio>
                    <el-radio label={1}>非铂金公会</el-radio>
                </el-radio-group>
                <el-table v-loading={isTableLoading.value} data={tableData.value.slice((currentPage.value - 1) * pageSize.value, currentPage.value * pageSize.value)} stripe style="width: 100%">
