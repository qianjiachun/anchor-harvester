...
export default defineComponent({
    ...
    setup(props, ctx) {
        ...
        const exportExcel = () => {
            ...
            let header = ["分区1", "分区2", "房间号", "昵称", "UID", "性别", "直播间标题", "直播间公告", "关注数", "热度", "月弹幕人数", "月流水", "最后更新时间"];
            let body: any = [];
            for (let i = 0; i < tableData.value.length; i++) {
                let item = tableData.value[i];
                let temp = [];
                temp.push(zone1, zone2, item.rid, item.nn, item.uid, item.gender, item.title, item.notice, item.fans, item.hot, item.month_barrage_num, item.month_gift_price, item.update_time);
                body.push(temp);
            }
        ...
        return () => (
            <>
                ...
                <el-table data={tableData.value.slice((currentPage.value - 1) * pageSize.value, currentPage.value * pageSize.value)} stripe style="width: 100%">
                    ...
                    <el-table-column sortable prop="month_barrage_num" label="月弹幕人数" width="120" align="center"></el-table-column>
                    <el-table-column sortable prop="month_gift_price" label="月流水" width="120" align="center"></el-table-column>
